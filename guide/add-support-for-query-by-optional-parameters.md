# Adding support for Query by Optional Parameter #

**Step 1** - Now we will be adding support for querying by entity property. In this particular case we can query account by firstname, lastname or email. Let's add support for querying by firstname only. For this modify our QueryAccount struct and add another optional property.
```
...
**Step 2** - Open src/main.rs and modify our Info struct to add optional property support. We need to make firstname property optional as we might not want to filter by firstname **all the time**.
...
**Step 5** - Save your changes and execute following command to start the application.
...
**Step 7** - Also verify that Account Rest service still returns results when firstname is not used.
```
